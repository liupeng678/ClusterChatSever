...
11. 我们可以将一个cpp文件的多个头文件打包成自己的头文件，然后直接引用自己的头文件就行。
...
2. redis的发布订阅任务底层不熟练
3. 分布式中群聊天没有实现，我们多表查询到用户之后，依次去发，这样做一下估计可以的， 但是由于时间原因没有实现。


### 分布式编程问题: proxyservice和用户这两个服务编写过程中的问题
1. 首先我们不同的probuff的命名空间是不一样的， 因此不同担心变量名冲突问题。 对于各种服务节点我们都定义了不同的probuff协议。 这些我们都是提前设置好得， 每个服务就知道它是那种协议， 不是自动感应的。
2. 其实网关中有很多调用stub, 这些都是rpc调用设置的， 但是对于聊天服务没有用rpc调用是没有用这个stub的。 网关是多个rpc调用者。
3. 我们网关服务器调用的时候如果不需要返回或者判断， response或者controller是可以去掉的。 请求的stub会去zk里面找这个stub类名 + 方法名的ip， 然后按照 整体头的大小+ rpc头 + 大小+ 数据（我们自己想实际配置的）的格式发送过去。 
